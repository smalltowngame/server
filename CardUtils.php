...
class CardUtils {
    ...
    public function getPlayer($selects = null, $id = null) {
        ...
        if (!$id) {
            if (!self::$userId) {
                echo "Bad use of cardUtils->getPlayer. not self:userId is defined when attrs: " . json_encode($selects);
                return false;
            }
            $id = self::$userId;
        }
        ...
        $result = petition("$sql FROM plays, players WHERE gameId = $gameId AND userId = :userId", $values)[0];
