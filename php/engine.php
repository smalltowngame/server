...
//                    $this->updateGame(null, "status");
        ...
        //update all players as waiting message (not dead)
        sql("UPDATE smltown_plays SET message = '' WHERE message = NULL AND status > -1");
        return false;
    }
...
//        echo "messageReceived";
        ...
        $sth = sql("UPDATE smltown_plays SET message = NULL WHERE message <> '' AND id = $playId");
        if ($sth->rowCount() == 0) {
            //DEBUG
            //$this->setError("error: no message to recieve.");
            return; //any message to recibe, prevent posible hack or error
        }
    ...
    public function selectPlayer() {
        ...
        sql("UPDATE smltown_plays SET sel = :selectedId, message = NULL WHERE id = $playId", $values);
        $this->selectPlayerResponse();
    }

    public function unSelectPlayer() {
        $playId = $this->playId;
        sql("UPDATE smltown_plays SET sel = null, message = '' WHERE id = $playId");
        $this->selectPlayerResponse();
    }
...
//        echo "setNextStatus";
        ...
        if ($statusCount == $status) { //equals, not last
            $status = 1;
