...
trait Utils {
    ...
    //notification
    protected function setNotifications($message, $wheres = null, $excludeId = null) {
        $players = $this->getPlays($wheres);
        for ($i = 0; $i < count($players); $i++) {
            $playId = $players[$i]->id;
            if ($excludeId != $playId) {
    ...
    protected function playersAlive($onlyRealPlayers = false) {
        ...
        $sql = "SELECT count(*) as count FROM smltown_plays WHERE gameId = $gameId AND status > 0 AND admin != -1";
        //    if ($onlyRealPlayers) {
        //        $sql = "$sql AND admin != -2";
        //    }
    ...
    protected function checkGameErrors() {
        ...
        if ($sth->rowCount() > 0) {
            $this->setError("warn: fixed bot error");
        }
    }
...
//    }
    ...
    protected function loadCards($all = false) { //GAME ID FUTURE ERRORS
        ...
        if (!$all) { // if specific game
            ...
            if (empty($string)) {
                ...
                echo "SMLTOWN.Message.flash('_noCardsSelected')";
                return false;
            }
            try {
    ...
    protected function saveMessage($message, $playId = null, $exclude = null) {
        $gameId = $this->gameId;
